...
def gerar_dim_clientes(n_clientes=1_000_000):
    ...
    # 🔧 Conversão explícita para datetime64[ns]
    df["data_nascimento"] = pd.to_datetime(df["data_nascimento"]).dt.normalize()
    df["data_criacao"] = pd.to_datetime(df["data_criacao"]).dt.normalize()

    caminho = os.path.join(BASE_DIR, "dim_clientes.parquet")
    df.to_parquet(caminho, index=False, engine="fastparquet")
...
def gerar_dim_lojas(n_lojas=5_000):
    ...
    # 🔧 Conversão explícita
    df["data_abertura"] = pd.to_datetime(df["data_abertura"]).dt.normalize()
...
def gerar_fato_vendas(
    ...
    """
    Gera fato_vendas em vários arquivos parquet.
    """

    dim_clientes = pd.read_parquet(
        os.path.join(BASE_DIR, "dim_clientes.parquet")
    )[["id_cliente"]]

    dim_produtos = pd.read_parquet(
        os.path.join(BASE_DIR, "dim_produtos.parquet")
    )[["id_produto", "preco_unitario"]]

    dim_lojas = pd.read_parquet(
        os.path.join(BASE_DIR, "dim_lojas.parquet")
    )[["id_loja"]]
    ...
    preco_map = dict(
        zip(dim_produtos["id_produto"].values,
            dim_produtos["preco_unitario"].values)
    )
    ...
    for chunk_idx in range(n_chunks):
        ...
        df_chunk = pd.DataFrame(
            {
                "id_venda": range(
                    linha_atual + 1,
                    linha_atual + 1 + linhas_nesse_chunk
                ),
        ...
        caminho_chunk = os.path.join(
            fato_dir,
            f"fato_vendas_chunk_{chunk_idx:04d}.parquet"
        )

        df_chunk.to_parquet(caminho_chunk, index=False, engine="fastparquet")
        ...
        print(
            ...
            f"{caminho_chunk} "
            f"({linhas_nesse_chunk:,} linhas, "
            f"total gerado: {linha_atual:,})"
        )
